...
export default class ChecklistComplexityService {
    ...
    getCoefficientWorker(productData, worker) {
        // console.log("cbGetProductData = ", productData);
        let workerCoefficient = 0;
        for (const question of this.questions) {
            // console.log('question = ', question.coefficients[worker],worker, question);
            if (question.isQuestion) {
                if (question.answer) {
                    workerCoefficient += +question.coefficients[worker] || 0;
                }
            } else if (question.productField === 'numberModules') {
                const numberModules = productData.questions?.[question.productField] || 0;
                if (numberModules > 1) {
                    workerCoefficient += question.coefficients[worker] * numberModules;
                }
            } else {
